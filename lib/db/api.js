...
dbapi.saveDeviceIdentity = function(serial, identity) {
  ...
  , cpuPlatform: identity.cpuPlatform  // @HY 2017-12-10  add cpu platform into DB to fulfill requirement from GG
    , brand: identity.brand            // @HY 2017-12-10  add cpu platform into DB to fulfill requirement from GG
...
dbapi.loadDevicesBySelector = function(selector) {
  return db.run(r.table('devices').filter(selector))
}
...
dbapi.loadDeviceByInventoryID = function (id) {
  return db.run(r.table('devices').filter({'inventoryID':id}))
}

dbapi.loadDeviceBySelector = function(selector) {
  return db.run(r.table('devices').filter(selector))
}
...
dbapi.addRemoteTask = function(testID, serial, owner, op, status, error) {
  console.log(testID, serial, op, status, error)
  return db.run(r.table('remoteTasks').insert({
    id: testID
    , serial: serial
    , owner: owner
    , LastOperation: op
...
dbapi.updateRemoteTask = function(testID, serial, op, status, storage, error) {
  console.log(testID, serial, op, status, storage, error)
  return db.run(r.table('remoteTasks').get(testID).update({
      storage: storage
    , status: status
    , LastOperation: op
    , comments: error
    ...
    .then(function(stats) {
      if (stats.skipped) {
        log.warn("Failed to find record in DB for remote task with ID=", testID)
      }
      return stats
    })
}
