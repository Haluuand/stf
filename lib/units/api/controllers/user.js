...
, deleteUserDeviceBySelector: deleteUserDeviceBySelector
  ...
  dbapi.loadDevice(serial)
    .then(function(device) {
      ...
      if (!deviceutil.isOwnedByUser(device, req.user)) {
        return res.status(403).json({
          success: false
        , description: util.format('You cannot release this device %s. Not owned by you', serial) 
        })
      }
...
function deleteUserDeviceBySelector(req, res) {
  var selectorText = req.swagger.params.selector.value
  var selector = {}
  ...
  try {
    selector = JSON.parse(selectorText)
          ...
          res.status(500).json({
            success: false
            , description: "Failed to parse JSON in parameter of selector"
          })
          return 
  }

  dbapi.loadDevicesBySelector(selector)  
    .then(function(cursor) {
      return Promise.promisify(cursor.toArray, cursor)()
        .then(function(deviceList) {
          ...
          if (!deviceutil.isOwnedByUser(device, req.user)) {
            return res.status(403).json({
              success: false
            , description: `You cannot release this device with serial ${serial}. Not owned by you(${req.user.email})`
            })
          }
...
function remoteConnectUserDeviceBySerial(req, res) {
  ...
  dbapi.loadDevice(serial)
    .then(function(device) {
      ...
      var inventoryID = device.inventoryID
...
function addAdbPublicKey(req, res) {
  ...
  adbkit.util.parsePublicKey(data.adb_key)
    .then(function(key) {
    ...
    .catch(dbapi.DuplicateSecondaryIndexError, function() {
      ...
      return res.status(500).json({
        success: false
      , message: 'Unable to insert new adb key fingerprint to database: ' + err.message
      })
    })
}
