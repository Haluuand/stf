...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, apk) {
    ...
    function callService(intent) {
      console.log("++++ Run shell cmd: ", util.format(
    ...
    function openService() {
      ...
      // 2019-01-25 @thinkhy on some devices like Meizu Pro 7, need to start activity first then have chance to start service
      return adb.shell(options.serial, 'am start -n jp.co.cyberagent.stf/.IdentityActivity')
        .timeout(15000)
        .then(function() {
        ...
        .delay(2000)
        .then(function() {
          // 2019-01-25  @thinkhy make the activity run in the background
          return adb.shell(options.serial, 'input keyevent 3')
        })
        .then(function() {
    ...
    plugin.getDisplay = function(id) {
     ...
     var resolution = {}
     return devutil.getResolution(adb, options.serial)
       .then(function(res) {
                resolution = res
                return runServiceCommand(apk.wire.MessageType.GET_DISPLAY
                                     ,new apk.wire.GetDisplayRequest(id))
            })
        .then(function(data) {
          var response = apk.wire.GetDisplayResponse.decode(data)

          // we extract screen size from output of "wm size" first
