...
module.exports = db.ensureConnectivity(function(options) {
  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.JoinGroupByAdbFingerprintMessage, function(channel, message) {
      ...
      dbapi.lookupUserByAdbFingerprint(message.fingerprint)
        .then(function(user) {
          ...
          // DEBUG by HY [ 2017-11-12 ]
          /*
          log.info("+++++ In JoinGroupByAdbFingerprintMessage, current user: ", user)
          log.info("+++++ In JoinGroupByAdbFingerprintMessage, current Group: ", message.currentGroup)
    ...
    .on(wire.TransactionDoneMessage, function(channel, message, data) {
      log.info('++++ [DEBUG] TransactionDoneMessage channel: %s, message: %s', channel, message)
      appDealer.send([channel, data])
      ...
      try {
        ...
        log.error('failed to parse message body in processor: ', err)
      }

      if (msg === null || (typeof msg.action === 'undefined')) {
        return
      }

      if (msg.action === GTF_MSG_RESP_START_TCPDUMP) {
        if (typeof msg.status !== 'undefined' && msg.status === 'success') {
          dbapi.addRemoteTask(msg.testID, msg.serial, msg.ownerEmail, msg.action, msg.status, msg.error)
        }
      }
      else if (msg.action === GTF_MSG_RESP_STOP_TCPDUMP) {
        dbapi.updateRemoteTask(msg.testID, msg.serial, msg.action, msg.status, msg.url, msg.error)
      }
    })
