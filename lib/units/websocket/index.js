    ...
    new Promise(function (resolve) {
      ...
      // Device note
        ...
        .on('device.install', function (channel, responseChannel, data) {
          ...
          // TODO: record install package
          // dbapi.saveInstallRecord(user, package) 2018-03-18 thinkhy
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
        ...
        .on('logcat.stop', function (channel, responseChannel) {
          // logcatRateLimiter.tryRemoveTokens(99999)
          // logcatRateLimiter = null
        ...
        .on('tcpdump.start', function (channel, responseChannel, data) {
          joinChannel(responseChannel)
          log.info("+++ [DEBUG] websocket received msg tcpdump.start")
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.StartTcpdumpMessage(data)
            )
          ])
        })
        .on('tcpdump.stop', function (channel, responseChannel, data) {
          joinChannel(responseChannel)
          log.info("+++ [DEBUG] websocket received msg tcpdump.stop")
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.StopTcpdumpMessage(data)
            )
          ])
        })
    })
